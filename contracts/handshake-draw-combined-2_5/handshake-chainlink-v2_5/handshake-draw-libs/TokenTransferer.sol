...
contract TokenTransferer {
    ...
    function _cacheAndPerformMultiERC20TransferCustom(address token, address[] memory recipients, uint256[] memory winnerIds, uint256 winningAmount, bytes32 drawId_, uint256 slot) internal {
        bytes32 cache;
        assembly {
            cache := mload(0x40)
        }
