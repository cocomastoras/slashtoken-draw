...
contract TokenTransferer {
    ...
    function _cacheMemoryAndPerformMultiERC20TransferCustom(address token, address[] memory recipients, uint256[] memory winnerIds, uint256 winningAmount, bytes32 drawId_, uint256 slot) internal {
        bytes32 memcache;
        assembly {
            memcache := mload(0x40)
        }
        _performMultiERC20TransferCustom(token, recipients, winnerIds, winningAmount, drawId_, slot);
        assembly {
            mstore(0x40, memcache)
        }
    }
    function _performMultiERC20TransferCustom(address token, address[] memory recipients, uint256[] memory winnerIds, uint256 winningAmount, bytes32 drawId_, uint256 slot) internal {
         ...
         assembly {
                ...
                )
            {
             ...
             for { let i := 0 } lt(i, recipientsLength) { i := add(i, 1) } {
                ...
                let winnerPosition := sload(finalKey)
...
//                 Perform the transfer, reverting upon failure.
    ...
    function _performNativeCustom(address[] memory recipients, uint256[] memory winnerIds, uint256 winningAmount, bytes32 drawId_, uint256 slot) internal {
        /// @solidity memory-safe-assembly
         assembly {
            let recipientsLength := mload(recipients)
    ...
    function _performMultiERC721Transfer(address token, address[] memory recipients, uint256[] memory winnerIds, uint256[] memory erc721Ids, bytes32 drawId_, uint256 slot) internal {
        /// @solidity memory-safe-assembly
         assembly {
             let recipientsLength := mload(recipients)
             let from := caller()
