...
/// @custom:version 1.0
/// @title SlashTokenDrawPythV1
/// @notice SlashTokenDrawPythV1 is a simple, yet powerful tool to airdrop tokens/NFTs or native currency to lottery winners using merkle trees and pyth's vrf.
contract SlashTokenDrawPythBlV1 is ManagerActions, VRFConfig, TokenTransferer {
    constructor(address entropy, address registry_)ManagerActions(registry_)VRFConfig(entropy){}
    ...
    /**
        ...
        @param commitment The operator's secret number commitment
        @param fee The entropy fee
    ...
    **/
    function createNewErc20Draw(bytes32 drawId_, address operator, address erc20Token, bytes32 rootHash, uint256 numOfWinners, uint256 numOfParticipants, uint256 winningAmountPerUser, bytes32 commitment, uint256 fee) external payable {
        ...
        _requestRandomWords(drawId_, commitment, fee);
    }
    ...
    /**
        ...
        @dev Initialises a new native draw using Pyth's VRF
    ...
    **/
    ...
    /**
        ...
        @dev Initialises a new erc721 draw using Pyth's VRF
    ...
    **/
    function distributeErc20Prizes(bytes32 drawId_, bytes32[] memory proof, address[] memory winnerAddresses, uint256[] memory winnerIds, bool[] memory flags) external {
        ...
        require(drawIdToRandomNumber[drawId_] != 0, 'NVR');
        ...
        uint256 slot;
        assembly {
            slot := drawIdToWinnerPosition.slot
        }
        _cacheMemoryAndPerformMultiERC20TransferCustom(token, winnerAddresses, winnerIds, drawIdToWinningAmount[drawId_], drawId_, slot);
    }
    ...
    **/
    function distributeNativePrizes(bytes32 drawId_, bytes32[] memory proof, address[] memory winnerAddresses, uint256[] memory winnerIds, bool[] memory flags) external payable {
        ...
        _performNativeCustom(winnerAddresses, winnerIds, winningValue, drawId_, slot);
    }
    ...
    **/
    function distributeErc721Prizes(bytes32 drawId_, bytes32[] memory proof, address[] memory winnerAddresses, uint256[] memory winnerIds, bool[] memory flags, uint256[] memory erc721Ids) external {
        ...
        _performMultiERC721Transfer(token, winnerAddresses, winnerIds, erc721Ids, drawId_, slot);
    }
}
